#!/usr/bin/env python3
"""
Merge two semantic map .npy files and convert to PLY format.
Input 1 points are colored GREEN, Input 2 points are colored RED.
"""
...
def save_topdown_view(points, colors, labels, output_file, downsample_factor=1):
    ...
    if downsample_factor > 1:
        ...
        print(f"  Downsampled from {len(points)} to {len(points_plot)} points (factor: {downsample_factor})")
    else:
        points_plot = points
        colors_plot = colors
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Merge two semantic map .npy files to PLY format with colors (input1=green, input2=red)")
    ...
    parser.add_argument("--input_postfix1", type=str, default="sem_map_orig_utm",
                       help="Input file postfix (default: sem_map_orig_utm)")
    
    # Original .npy file postfix to compare against (default: same as input)
    parser.add_argument("--input_postfix2", type=str, default="sem_map_relabeled_utm",
                       help="Original postfix for .npy file to compare against (default: sem_map_orig_utm)")

    # Original .npy file postfix to compare against (default: same as input)
    parser.add_argument("--output_postfix", type=str, default="sem_map_merged_utm",
                       help="Original postfix for .npy file to compare against (default: sem_map_orig_utm)")
    ...
    # Downsample factor for top-down view
    parser.add_argument("--downsample", type=int, default=1,
                       help="Downsample factor for top-down view (1 = no downsampling, 2 = every 2nd point, etc.)")
    ...
    input_file1 = os.path.join(file_dir, f"{environment}_{args.input_postfix1}.npy")
    input_file2 = os.path.join(file_dir, f"{environment}_{args.input_postfix2}.npy")
    output_file = os.path.join(file_dir, f"{environment}_{args.output_postfix}.ply")
    
    print(f"\n{'='*80}")
    print("Merging Two Point Clouds")
    print(f"{'='*80}")
    print(f"Input 1 (will be GREEN): {input_file1}")
    print(f"Input 2 (will be RED): {input_file2}")
    print(f"Output: {output_file}")
    
    # Load first semantic map
    print(f"\n{'='*80}")
    print("Loading Input 1 (GREEN)")
    print(f"{'='*80}")
    points1, intensities1, labels1 = load_semantic_map(input_file1)
    
    # Load second semantic map
    print(f"\n{'='*80}")
    print("Loading Input 2 (RED)")
    print(f"{'='*80}")
    points2, intensities2, labels2 = load_semantic_map(input_file2)
    ...
    print(f"  Input 1: {len(points1)} points")
    print(f"  Input 2: {len(points2)} points")
    print(f"  Merged: {len(merged_points)} points")
    
    # Create colors: green for input1, red for input2
    print(f"\n{'='*80}")
    print("Assigning Colors")
    print(f"{'='*80}")

    green_colors = np.ones((len(points1), 3), dtype=np.float32) * [1.0, 165.0/255.0, 0.0]
    red_colors = np.ones((len(points2), 3), dtype=np.float32) * [0.5, 0.5, 0.5]
    
    merged_colors = np.vstack([green_colors, red_colors])
    print(f"  Input 1 points: ORANGE (RGB: [255, 165, 0])")
    print(f"  Input 2 points: GRAY (RGB: [128, 128, 128])")
    print(f"  Total colors: {len(merged_colors)}")
    
    # Save to PLY
    print(f"\n{'='*80}")
    print("Saving to PLY Format")
    print(f"{'='*80}")
    success = save_pointcloud_to_ply(merged_points, merged_colors, output_file)
    ...
    if success:
        ...
        print(f"{'='*80}")
        is_valid = validate_ply_file(output_file, len(merged_points))
        ...
        if is_valid:
            ...
            print(f"  - Input 1 (GREEN): {len(points1):,} points")
            print(f"  - Input 2 (RED): {len(points2):,} points")
        else:
            print(f"\n{'='*80}")
            print("WARNING!")
            print(f"{'='*80}")
