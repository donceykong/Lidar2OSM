...
def relabel_scan_using_nearest_neighbors(scan_points_world, kdtree, global_map_labels, max_distance=2.0):
    """
...
def process_robot_scans(input_global_map_file, dataset_path, environment, robot, 
    ...
    # Load robot poses
    poses_file = os.path.join(dataset_path, environment, robot, f"{robot}_{environment}_gt_utm_poses.csv")
    ...
    # Setup paths
    lidar_path = os.path.join(dataset_path, environment, robot, "lidar_bin/data")
    lidar_files = sorted(list(Path(lidar_path).glob("*.bin")))
    ...
    output_dir = os.path.join(dataset_path, environment, robot, f"{robot}_{environment}_refined_osm_lidar_labels")
    ...
    for frame_idx in tqdm(range(num_scans_to_process), desc=f"Relabeling {robot} scans"):
        ...
        # Load lidar scan
        scan_file = os.path.join(lidar_path, lidar_files[frame_idx])
...
def main():
    ...
    if args.robot:
        ...
        if not os.path.exists(env_path):
            ...
            return
        robots = [d for d in os.listdir(env_path) if os.path.isdir(os.path.join(env_path, d)) and d.startswith('robot')]
