...
class LaserScan:
    ...
    def get_img_stds(self):
        img_stds = torch.tensor(
            [
                [[np.std(self.depth_list)]],
    ...
    def get_img_means(self):
        img_means = torch.tensor(
            [
                [[np.mean(self.depth_list)]],
    ...
    def do_range_projection(self):
        ...
        # depth = np.linalg.norm(self.points, 2, axis=1)
        depth = (
            np.linalg.norm(self.points, 2, axis=1) + 0.00000000000000000000000000000001
        )  # 2
