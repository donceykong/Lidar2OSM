...
# Internal
from lidar2osm.visualize.data_statistics import report_size_extracted_data
from core.robot import robot
from Lidar2OSM.lidar2osm.core.pointcloud.laserscan_new import birdseye_to_point_cloud, point_cloud_2_birdseye
...
def vis_lidar_BEV(seq_extract):
    ...
    while current_frame < last_extracted_frame:
        ...
        if os.path.exists(frame_build_points_file):
            ...
            rgb_np_reshaped = rgb_np.reshape(
                (lidar_points_all_BEV.shape[0], lidar_points_all_BEV.shape[1], 3)
            )
    ...
    rgb_np = labels2RGB(lidar_points_bev_accum[:, 4], seq_extract.labels_dict)
    lidar_points_bev_o3d.colors = o3d.utility.Vector3dVector(rgb_np)
...
with open("config/o3d_color_palette.yaml", "r") as file:
    ...
    while seq_extract.current_frame < last_extracted_frame:
        ...
        if os.path.exists(frame_build_points_file):
            ...
            if extracted_data is not None:
                ...
                labels_np = read_bin_file(pc_frame_label_path, dtype=np.int16, shape=(-1)) # Note: use int32 for inferred labels
