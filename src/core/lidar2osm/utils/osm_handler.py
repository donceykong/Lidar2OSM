...
class OSMDataHandler:
    ...
    # def plot_road_network(self, poses_latlon: np.ndarray, ax: Optional[plt.Axes] = None) -> bool:
    #     """
    #     Plot filtered OSM road network.
    ...
    #     filtered_nodes = self.filter_nodes_by_distance(poses_latlon)
        
    #     if not filtered_nodes:
    #         print("No OSM nodes found within filter distance")
    #         return False
        
    #     # Create subgraph with only nearby nodes
    #     G_filtered = self.osm_graph.subgraph(filtered_nodes)
    #     print(f"    - Filtered OSM graph has {len(G_filtered.nodes)} nodes and {len(G_filtered.edges)} edges")
        
    #     # Color edges based on OSM tags
    #     edge_colors = []
    #     edge_widths = []
    ...
    #         if highway in ['primary', 'secondary', 'tertiary', 'residential', 'unclassified', 'road']:
    #             edge_colors.append('red')
    #             edge_widths.append(2.0)
    #         elif highway in ['footway', 'pedestrian', 'sidewalk', 'path']:
    #             edge_colors.append('red')
    ...
    #     # Plot the filtered OSM network
    #     ox.plot_graph(G_filtered, ax=ax, show=False, close=False, 
    #                  node_color='lightblue', node_size=1, 
    ...
    def plot_all_osm_data(self, poses_latlon: np.ndarray, ax: Optional[plt.Axes] = None,
                         ...
                         plot_roads: bool = True, plot_buildings: bool = True,
        ...
        Args:
            poses_latlon: Array of poses in lat/lon format
            ax: Matplotlib axes to plot on. If None, uses current axes.
            plot_roads: Whether to plot road network
        ...
        # if plot_roads:
        #     if self.plot_road_network(poses_latlon, ax):
        #         success = True
