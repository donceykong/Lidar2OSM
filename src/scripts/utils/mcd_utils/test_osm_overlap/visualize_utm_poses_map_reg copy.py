...
def build_map_from_poses(sequence_path, pose_csv, 
    ...
    gt_labels_dir = sequence_path / "inferred_labels"
    ...
    if poses_dict:
        ...
        print(f"  Position (world): [{initial_pose['position'][0]:.2f}, {initial_pose['position'][1]:.2f}, {initial_pose['position'][2]:.2f}]")
...
if __name__ == "__main__":
    ...
    points, labels, initial_pose = build_map_from_poses(
        sequence_path, 
        pose_csv,
    ...
    if points is not None and labels is not None:
        visualize_semantic_map(points, labels, show_osm=show_osm, osm_file_path=osm_file_path,
                              initial_pose=initial_pose, initial_latlon=initial_latlon)
    else:
