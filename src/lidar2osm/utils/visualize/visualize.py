...
from lidar2osm.config_loader import get_config_dir
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help='Dataset config YAML (defaults to `config/internal/data_cfg_cu_multi.yaml`).',
    )
    parser.add_argument(
        '--what', '-w',
        type=str,
    ...
    if FLAGS.what == "kitti":
        ...
        from lidar2osm.utils.visualize.laserscanvis import LaserScanVis
    elif FLAGS.what == "poss":
        from common.posslaserscan import LaserScan, SemLaserScan  # type: ignore[import-not-found]
        from common.posslaserscanvis import LaserScanVis  # type: ignore[import-not-found]
    else:
        raise TypeError("This type dataset doesn't exist (use kitti or poss)! Exiting...")
    # open config file
    try:
        cfg_dir = get_config_dir()
        data_cfg_path = FLAGS.data_cfg or str(cfg_dir / "internal/data_cfg_cu_multi.yaml")
        print(f"Opening data config file: {data_cfg_path}")
        CFG = yaml.safe_load(open(data_cfg_path, 'r'))
