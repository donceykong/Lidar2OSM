...
def visualize_geodesic_path(mesh, path, line_width=100):
    """
    ...
    Parameters:
        ...
        line_width (int): The thickness of the geodesic path lines.
    """
    ...
    line_set.colors = o3d.utility.Vector3dVector([[0, 1, 0] for _ in path_edges])  # Red for path

    # Visualize the mesh and the geodesic path with line width
...
#     triangulation = pcl.Surface.GreedyProjectionTriangulation.PointXYZ()
#     mesh = triangulation.reconstruct(cloud)
...
def make_mesh(pcd):
    ...
    mesh, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(
        pcd, depth=10  # Depth controls detail level; increase for finer meshes
    )

    # Compute density threshold
    density_threshold = np.percentile(densities, 5)  # Keep top 95% dense areas
    ...
    # pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=1, max_nn=50))
    # # Create a mesh using Ball-Pivoting
    # radii = [0.5, 0.5, 0.5]  # Radii for ball-pivoting
    # mesh = o3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(pcd, o3d.utility.DoubleVector(radii))
    # o3d.io.write_triangle_mesh("/media/donceykong/donceys_data_ssd/KL_robot1_mesh.ply", mesh)  # Save as PLY
...
def align_maps(target_sem_pc, source_sem_pc, stats_obj, tf_ground_truth, iter):
    ...
    sem_labels_3x3 = np.stack([target_sem_pc[:, 3]] * 3, axis=-1)

    print("Creating initial pc")
    target_normals = target_sem_pc[:, 4:7]

    target_pcd = o3d.geometry.PointCloud()
    target_pcd.points = o3d.utility.Vector3dVector(target_sem_pc[:, :3])
    target_pcd.colors = o3d.utility.Vector3dVector(sem_labels_3x3)
    ...
    print(f"len(points): {len(target_pcd.points)}")

    print("Labels2RGB")
    semantic_pc_ds = np.zeros((len(target_pcd.points), 4))
    semantic_pc_ds[:, 3] = np.asarray(target_pcd.colors, dtype=np.int16)[:, 0]
    semantic_pc_ds_rgb = labels2RGB(semantic_pc_ds[:, 3], labels_dict)

    print("Setting up point cloud")
    target_pcd2 = o3d.geometry.PointCloud()
    target_pcd2.points = o3d.utility.Vector3dVector(target_pcd.points)
    target_pcd2.colors = o3d.utility.Vector3dVector(semantic_pc_ds_rgb)
    target_pcd2.normals = o3d.utility.Vector3dVector(target_pcd.normals)
    o3d.visualization.draw_geometries([target_pcd2])

    # target_pcd_voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(target_pcd2, voxel_size=voxel_size)
    # o3d.visualization.draw_geometries([target_pcd_voxel_grid])

    # Make and solve mesh
    print("Creating mesh")
    mesh = make_mesh(target_pcd2)
    print("Solving shortest path")
    solve_shortest_path(mesh, source_idx=10, target_idx=8000)
...
def get_accum_points(
    ...
    for frame_idx in tqdm(range(first_frame_idx, final_frame_idx, frame_increment)):
        ...
        # # Filter points where distance is greater than  5m but less than 50m
        distances = np.linalg.norm(pc_xyz, axis=1)
        pc_xyz = pc_xyz[distances > 2]
        labels_np = labels_np[distances > 2]

        distances = np.linalg.norm(pc_xyz, axis=1)
        pc_xyz = pc_xyz[distances < 30]
        labels_np = labels_np[distances < 30]
        ...
        pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(
            radius=0.1, max_nn=5))  # Adjust parameters as needed
        normals_list.extend(np.asarray(pcd.normals))
        normals = np.asarray(pcd.normals)
        semantic_points_with_normals = np.concatenate([transformed_xyz, labels_np.reshape(-1, 1), normals], axis=1)
        ...
        semantic_points_list.extend(semantic_points_with_normals)
