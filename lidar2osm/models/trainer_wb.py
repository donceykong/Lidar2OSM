...
class Trainer:
    def __init__(self, ARCH, DATA, dataset_name, datadir, logdir, path=None):
        # parameters
        self.ARCH = ARCH
        self.DATA = DATA
        ...
        epsilon_w = self.ARCH["train"]["epsilon_w"]  # Must be bias weight... - Doncey
        ...
        save_to_log(
            self.log,
            "model.txt",
            "Number of parameters: %.5f M" % (pytorch_total_params / 1000000),
        )
        ...
        if self.path is not None:
            ...
            w_dict = torch.load(
                path + "/SENet_valid_best", map_location=lambda storage, loc: storage
            )
    ...
    def make_log_img(depth, mask, pred, gt, color_fn):
        ...
        depth_img = (
            cv2.applyColorMap(depth, Trainer.get_mpl_colormap("viridis"))
            * mask[..., None]
        )
        ...
        out_img = np.concatenate([depth_img, pred_color, gt_color], axis=0)
        ...
        for i, (
            ...
            if i % self.ARCH["train"]["report_batch"] == 0:
                ...
                wandb.log({
                    ...
                    "train/iou": iou.val,
                    "train/iou_avg": iou.avg,
                    "train/boundary_loss": bd.val,
                    "train/boundary_loss_avg": bd.avg,
